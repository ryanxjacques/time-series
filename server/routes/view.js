...
view.get('/timeseries', async (req, res) => {
    ...
    const timeseries_res = await db.getRecordElement(connection, 'ts_metadata', fields, ts_id);
...
view.get('/users', async (req, res) => {
    ...
    const user_res = await db.getRecordElement(connection, 'users', fields, user_id);
    db.disconnect(connection);
    res.json(user_res);
});
